...
const baseGameResData: { [key: string]: gameResDataType } = {
  classic: {
    dice: [
      'classic_defaultA_',
      'classic_defaultA_',
...
const expansionsGameResData: { [key: string]: gameResDataType } = {
  none: {
    dice: [],
    goals: [],
    effective_dice: 0.5,
...
function setupDiceList({
  mainSetting,
  firstExpansion,
  secondExpansion,
}: GameSettings): string[] {
  return baseGameResData[mainSetting].dice.concat(
    expansionsGameResData[firstExpansion].dice,
    expansionsGameResData[secondExpansion].dice,
  );
}

function setupGoalsList({
  mainSetting,
  firstExpansion,
  secondExpansion,
  goalSetting,
  ...
  const getGoals = (expansion: string) =>
    expansion !== 'none' ? expansionsGameResData[expansion].goals : [];
  ...
  // Goals
  const goalsRow = document.getElementById('game-goals') as HTMLDivElement;
  ...
  for (let i = 0; i < myGoalsList.length; i++) {
    ...
    const goalCaption = document.createElement('p');
...
function setupBlueprint({
  ...
  if (blueprints && blueprints.length > 0) {
    const myBlueprint: string = randItem(blueprints);
...
function setRowLength({ mainSetting }: GameSettings): number {
